...
local function apply_effect(obj)
    if obj:IsA("BasePart") then
        local name = string.lower(obj.Name)
        if name:find("tree") or name:find("foliage") or name:find("grass") then
            obj.Transparency = current_alpha
            obj.CanCollide = (current_alpha < 1)
            obj.CastShadow = (current_alpha < 1)
        end
    end
end
...
WorldLeft:AddToggle("remove_grass", {Text = "Remove Grass", Callback = function(s) flags.RemoveGrass = s end})
