...
local BrightnessContrast = Lighting:FindFirstChild("BrightnessContrast")
...
local function apply_alpha(obj, alpha)
    if obj:IsA("BasePart") then
        local name = string.lower(obj.Name)
        if name:find("tree") or name:find("foliage") then
            obj.Transparency = alpha
            obj.CanCollide = (alpha < 1)
            obj.CastShadow = (alpha < 1)
        end
    end
end

workspace.DescendantAdded:Connect(function(obj)
    if flags.RemoveGrass and string.find(string.lower(obj.Name), "grass") then
        task.defer(function() hide_part(obj) end)
    end
    task.defer(function() apply_alpha(obj, current_alpha) end)
end)
...
RunService.RenderStepped:Connect(function()
    ...
    if flags.RemoveClouds then
        local clouds = Terrain:FindFirstChildOfClass("Clouds")
        if clouds then clouds.Enabled = false end
    end
    if flags.RemoveShadows then
...
local WorldLeft = Tabs.World:AddLeftGroupbox("World Settings")
local WorldRight = Tabs.World:AddRightGroupbox("Lighting & Foliage")

WorldLeft:AddToggle("remove_clouds", {Text = "Remove Clouds", Callback = function(s) flags.RemoveClouds = s end})
WorldLeft:AddToggle("remove_grass", {Text = "Remove Grass", Callback = function(s) 
    flags.RemoveGrass = s 
    if s then 
        for _, obj in pairs(workspace:GetDescendants()) do
            if string.find(string.lower(obj.Name), "grass") then hide_part(obj) end
        end
    end
end})
WorldLeft:AddToggle("remove_shadow", {Text = "Remove Shadow", Callback = function(s) flags.RemoveShadows = s end})
WorldLeft:AddToggle("remove_fog", {Text = "Remove Fog", Callback = function(s) flags.RemoveFog = s end})

WorldRight:AddSlider("foliage_alpha", {
    Text = "Foliage Alpha", Min = 0, Max = 1, Default = 0, Increment = 0.1, Rounding = 1,
    Callback = function(e)
        current_alpha = e
        for _, obj in pairs(workspace:GetDescendants()) do
            apply_alpha(obj, e)
...
WorldRight:AddToggle("fullbright", {Text = "Fullbright", Callback = function(s) flags.Fullbright = s end})

WorldRight:AddToggle("brightness_contrast", {
    Text = "Brightness Contrast", 
    Callback = function(s) 
        if BrightnessContrast then BrightnessContrast.Enabled = s BrightnessContrast.Brightness = 1 end 
    end
})

WorldRight:AddSlider("brightness", {
    Text = "Brightness", Min = 0, Max = 10, Default = 5, Rounding = 1,
    Callback = function(v) Lighting.Brightness = v end
})

WorldRight:AddSlider("time_of_day", {
    Text = "Time (h)", Min = 0, Max = 24, Default = 12, Rounding = 1,
