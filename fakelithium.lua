-- \\ Internal Drawing Library (Tối ưu riêng cho Lithium)
local Drawing = {}
...
-- Gắn vào PlayerGui để né lỗi "lacking capability Plugin" của CoreGui
pcall(function()
...
function Drawing.new(drawingType)
    ...
    local proxy = setmetatable({}, {
        ...
        __newindex = function(_, key, value)
            props[key] = value
            local rTrans = 1 - (props.Transparency or 1) -- Đảo ngược Transparency (Drawing API xài 1 là hiện)
            ...
            elseif key == "Transparency" then
                if drawingType == "Text" then
                    ...
                    if inst:FindFirstChild("UIStroke") then inst.UIStroke.Transparency = rTrans end
                elseif drawingType == "Square" then
                    inst.BackgroundTransparency = props.Filled and rTrans or 1
                    if inst:FindFirstChild("UIStroke") then inst.UIStroke.Transparency = rTrans end
                elseif drawingType == "Line" then
                    inst.BackgroundTransparency = rTrans
                end
            elseif key == "Color" then
                if drawingType == "Text" then inst.TextColor3 = value 
                elseif drawingType == "Square" and not props.Filled then 
                    inst.BackgroundColor3 = value
                    inst.UIStroke.Color = value 
                else inst.BackgroundColor3 = value end
            ...
            elseif key == "Center" and drawingType == "Text" then
                inst.AnchorPoint = value and Vector2.new(0.5, 0) or Vector2.new(0, 0)
            elseif key == "Outline" then
                local stroke = inst:FindFirstChild("UIStroke")
                if stroke then stroke.Enabled = value end
            elseif key == "OutlineColor" then
                local stroke = inst:FindFirstChild("UIStroke")
                if stroke then stroke.Color = value end
            elseif key == "Filled" and drawingType == "Square" then
                inst.BackgroundTransparency = value and rTrans or 1
                local stroke = inst:FindFirstChild("UIStroke")
                if stroke then stroke.Enabled = not value end
            elseif key == "Thickness" then
                local stroke = inst:FindFirstChild("UIStroke")
                if stroke and drawingType == "Square" then stroke.Thickness = value end
                if drawingType == "Line" and props.From and props.To then
                    inst.Size = UDim2.new(0, (props.To - props.From).Magnitude, 0, value)
                end
