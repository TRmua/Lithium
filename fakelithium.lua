...
-- Gắn vào PlayerGui để né lỗi "lacking capability Plugin"
pcall(function()
...
function Drawing.new(drawingType)
    ...
    elseif drawingType == "Text" then
        ...
        inst.Font = Enum.Font.Roboto -- Đổi lại font chữ chuẩn, mượt mà như ảnh 2
        inst.TextSize = 13
        local stroke = Instance.new("UIStroke", inst)
        stroke.Enabled = false
        stroke.Thickness = 1 -- Viền mượt tạo bóng (soft outline) giống ảnh 2
            ...
            elseif key == "Position" then
                if drawingType ~= "Line" then
                    if drawingType == "Text" and props.Center then
                        -- TRỪ ĐI 8 PIXEL (Y - 8) để đẩy chữ Tên lên cao tạo khoảng trống với hộp 
                        -- và kéo chữ Khoảng cách áp sát xuống đáy hộp (giống y hệt API mặc định)
                        inst.Position = UDim2.new(0, value.X, 0, value.Y - 8)
